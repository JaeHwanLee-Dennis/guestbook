...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="dennis114/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/JaeHwanLee-Dennis/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/JaeHwanLee-Dennis/guestbook-config.git'
                }
            }
        }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://dennis11483:${GITHUB_ACCESS_TOKEN}@github.com/JaeHwanLee-Dennis/guestbook-config.git"
                    '''
                }
            }
        }
